...
def detect_video(input_file, output_file):
    ...
    out_cap = cv2.VideoWriter(
        output_file, 0x7634706D, int(fps), (int(width), int(height))
    )
...
if __name__ == "__main__":
    ...
    opencv_model_path = (
        "./resource/opencv/res10_300x300_ssd_iter_140000_fp16.caffemodel"
    )
    ...
    parser.add_argument(
        "--detector", default="facenet", help="분석에 사용할 감지기를 고르세요(facenet, opencv)"
    )
    ...
    face_detector = (
        FacenetDetector()
        if res.detector == "facenet"
        else OpenCVFaceDetector(opencv_model_path, opencv_config_path)
    )  # opencv
    ...
    if os.path.isdir(res.input_file):
        for file in os.listdir(res.input_file):
            ...
            detect_video(
                os.path.join(res.input_file, file), os.path.join(res.output_file, file)
            )
    else:
