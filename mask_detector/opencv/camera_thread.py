...
class CameraThread(QThread):
    ...
    def run(self):
        ...
        nomaskIcon = cv2.imread(
            "./resource/image/remove_delete_close_1075.png", cv2.IMREAD_UNCHANGED
        )
            ...
            if ret:
                ...
                for i in range(detect.shape[0]):
                    ...
                    if (
                        ...
                        cv2.rectangle(
                            frame, (x1, y1), (x2, y2), (0, 0, 255), thickness=2
                        )
                        cv2.putText(
                            frame,
                            label,
                ...
                convertToQtFormat = QImage(
                    frame.data, w, h, bytesPerLine, QImage.Format_BGR888
                )
    ...
    def overlay(self, frame, nomaskIcon, pos):
        ...
        if (
            pos[0] + nomaskIcon.shape[1] > frame.shape[1]
            or pos[1] + nomaskIcon.shape[0] > frame.shape[0]
        ):
            return
        ...
        alpha = 1.0 - (nomaskIcon[:, :, 3] / 255.0)  # shape=(h, w)

        img1[:, :, 0] = (img1[:, :, 0] * alpha + img2[:, :, 0] * (1.0 - alpha)).astype(
            np.uint8
        )
        img1[:, :, 1] = (img1[:, :, 1] * alpha + img2[:, :, 1] * (1.0 - alpha)).astype(
            np.uint8
        )
        img1[:, :, 2] = (img1[:, :, 2] * alpha + img2[:, :, 2] * (1.0 - alpha)).astype(
            np.uint8
        )
